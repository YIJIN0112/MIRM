...
################################################
Reference:
This project code was implemented and enhanced based on the research findings published by Lin's team. For more details, please refer to their paper:“Improving Graph Collaborative Filtering with Neighborhood-enriched Contrastive Learning,” (doi:https://doi.org/10.1145/3485447.3512104.)
"""
...
class SVD_MI(GeneralRecommender):
    input_type = InputType.PAIRWISE

    def __init__(self, config, dataset):
        super(SVD_MI, self).__init__(config, dataset)
        ...
        # SVD
        if self.svd:
            self.U, self.value, self.V = torch.svd_lowrank(self.norm_adj_mat.cpu(),q=400,niter=30)
            self.svd_filter=self.weight_func(self.value[:self.latent_dim])
            self.user_vector = (self.U[:,:self.latent_dim]) * self.svd_filter
            self.item_vector = (self.V[:,:self.latent_dim]) * self.svd_filter
            self.user_embedding_ori, _ = torch.split(self.user_vector, [self.n_users, self.n_items])
            _, self.item_embedding_ori = torch.split(self.item_vector, [self.n_users, self.n_items])
            self.user_embedding = nn.Embedding.from_pretrained(self.user_embedding_ori, freeze=False)
            self.item_embedding = nn.Embedding.from_pretrained(self.item_embedding_ori, freeze=False)
        
        # BERT
        if self.mi > 0:
            self.content_nonlinear = nn.ModuleList([
                nn.Sequential(nn.Linear(768, self.latent_dim),
                             nn.ReLU())for i in range(self.mi)])
    ...
    def get_svd_embedding(self):
        
        svd_embeddings = torch.cat([self.user_embedding_ori, self.item_embedding_ori], dim=0)
        return svd_embeddings

    def forward(self):
        all_embeddings = self.get_ego_embeddings()
        if self.svd:
            svd_embeddings = self.get_svd_embedding()
        embeddings_list = [all_embeddings]
        for layer_idx in range(max(self.n_layers, self.hyper_layers*2)):
            all_embeddings = torch.sparse.mm(self.norm_adj_mat, all_embeddings)
            embeddings_list.append(all_embeddings)
        ...
        if self.svd:
            lightgcn_all_embeddings = torch.stack(embeddings_list[:self.n_layers+1]+[svd_embeddings.to(self.device)], dim=1)
        else:
            lightgcn_all_embeddings = torch.stack(embeddings_list[:self.n_layers+1], dim=1)
        lightgcn_all_embeddings = torch.mean(lightgcn_all_embeddings, dim=1)
    ...
    def calculate_loss(self, interaction, bert_emb=None):
        # clear the storage variable when training
        if self.restore_user_e is not None or self.restore_item_e is not None:
            self.restore_user_e, self.restore_item_e = None, None
        ...
        if self.mi>0:
            pos_item_content = bert_emb[pos_item].to(self.device)
            neg_item_content = bert_emb[neg_item].to(self.device)
        ...
        if self.mi > 0:
            ...
            final_neg_emb = []
            # # save attetion score
            # pos_attention_weights = []
            # neg_attention_weights = []
            for i in range(self.mi):
            #     # positive item
            #     pos_interest_vector = self.content_nonlinear[i](pos_item_content)
            #     # dot product
            #     pos_score = torch.matmul(pos_interest_vector, u_embeddings.T)
            #     pos_attention_weights.append(pos_score)
            #     final_pos_emb.append(pos_interest_vector)

            #     # negative item
            #     neg_interest_vector = self.content_nonlinear[i](neg_item_content)
            #     # dot production
            #     neg_score = -torch.matmul(neg_interest_vector, u_embeddings.T)  
            #     neg_attention_weights.append(neg_score)  
            #     final_neg_emb.append(neg_interest_vector)
            # pos_attention_weights = torch.nn.functional.softmax(torch.cat(pos_attention_weights, dim=1), dim=1)  
            # neg_attention_weights = torch.nn.functional.softmax(torch.cat(neg_attention_weights, dim=1), dim=1) 
            # # attention weight
            # final_pos_emb = torch.stack(final_pos_emb, dim=1)
            # final_neg_emb = torch.stack(final_neg_emb, dim=1)
            # weighted_pos_emb = torch.sum(pos_attention_weights.unsqueeze(2) * final_pos_emb, dim=1)
            # weighted_neg_emb = torch.sum(neg_attention_weights.unsqueeze(2) * final_neg_emb, dim=1)
            
                final_pos_emb.append(self.content_nonlinear[i](pos_item_content))
                final_neg_emb.append(self.content_nonlinear[i](neg_item_content))
            final_pos_emb = sum(final_pos_emb)/self.mi
            final_neg_emb = sum(final_neg_emb)/self.mi

            pos_embeddings = item_all_embeddings[pos_item] + weighted_pos_emb*self.lam
            neg_embeddings = item_all_embeddings[neg_item] + weighted_neg_emb*self.lam
        else:
            pos_embeddings = item_all_embeddings[pos_item] 
            neg_embeddings = item_all_embeddings[neg_item]
        ...
        return mf_loss + self.reg_weight * reg_loss, ssl_loss, proto_loss
        # return mf_loss, ssl_loss, proto_loss
