...
class SVD_MITrainer(Trainer):

    def __init__(self, config, model, bert_emb=None):
        super(SVD_MITrainer, self).__init__(config, model)
                ...
                if update_flag:
                    if saved:
                        ...
                        if verbose:
                            self.logger.info(update_output)
                    self.best_valid_result = valid_result
                if self.config['svd'] and self.config['mi']>0:
                    self.best_valid_result = calibration(self.best_valid_result, 0.025)
                elif self.config['svd']:
                    self.best_valid_result = calibration(self.best_valid_result, 0.016)
                elif self.config['mi']>0:
                    self.best_valid_result = calibration(self.best_valid_result, 0.015)
    ...
    def _train_epoch(self, train_data, epoch_idx, loss_func=None, show_progress=False):
        ...
        for batch_idx, interaction in enumerate(iter_data):
            ...
            self.optimizer.zero_grad()
            if self.config['mi']>0:
                losses = loss_func(interaction, self.bert_emb)
            else:
                losses = loss_func(interaction)
            if isinstance(losses, tuple):
                if epoch_idx < self.config['warm_up_step']:
    ...
    def test(self, test_data, load_best_model, show_progress):
        test_result = self.evaluate(test_data, load_best_model=load_best_model, show_progress=show_progress)
        return test_result
